...
import { activities, archivedActivities, committee, news, topGames } from '../src/data/editorial'
...
describe('routes éditoriales publiques', () => {
  it('relie les cinq rubriques depuis une entrée principale active par famille', async () => {
    ...
    const layout = await readFile(new URL('../src/components/editorial/EditorialLayout.astro', import.meta.url), 'utf8')
    expect(primaryNavigation.find((item) => item.label === 'Découvrir')?.href).toBe('/actualites')
  ...
  it('fournit une URL stable à chaque actualité et activité', () => {
    ...
    expect([...activities, ...archivedActivities].every((item) => item.slug && item.question)).toBe(true)
    expect(new Set([...news, ...activities, ...archivedActivities].map((item) => item.slug)).size).toBe(
      news.length + activities.length + archivedActivities.length,
    )
  })

  it('sépare les archives des activités à venir', () => {
    expect(activities.every((item) => item.status === 'demo')).toBe(true)
    expect(archivedActivities.every((item) => item.status === 'archive')).toBe(true)
  })

  it('limite la sélection à trois jeux et marque le contenu provisoire', () => {
    expect(topGames).toHaveLength(3)
    expect(topGames.every((game) => /confirmer|venir|sélectionner/.test(`${game.name} ${game.age}`))).toBe(true)
  })
  ...
  it('rend visibles les états sans contenu', async () => {
    ...
    expect(committeePage).toContain('Composition à confirmer')
  ...
  it('commence les pages par une question visiteur', async () => {
    ...
    expect(pages.every((page) => page.includes('question="') && page.includes('?"'))).toBe(true)
  })
})
